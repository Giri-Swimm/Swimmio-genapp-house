...
---
This document covers the <SwmToken path="base/src/lgapdb04.java" pos="124:3:3" line-data="    public LGAPDB04(Connection connection) {">`LGAPDB04`</SwmToken> class, focusing on:
...
2. Variables and functions defined in <SwmToken path="base/src/lgapdb04.java" pos="124:3:3" line-data="    public LGAPDB04(Connection connection) {">`LGAPDB04`</SwmToken>, with explanations and code citations for each function.
...
<SwmToken path="base/src/lgapdb04.java" pos="124:3:3" line-data="    public LGAPDB04(Connection connection) {">`LGAPDB04`</SwmToken> is a Java class designed for advanced actuarial premium calculations in commercial property insurance. It calculates insurance premiums by considering various actuarial factors, risk assessments, and coverage selections. The class encapsulates the entire premium calculation workflow, including data input, exposure calculations, rate loading from a database, application of modifiers, catastrophe and expense loading, discounts, taxes, and final premium computation. It is intended to be used as the main engine for premium computation in a commercial insurance context.

<SwmSnippet path="/base/src/lgapdb04.java" line="124">

---

The function <SwmToken path="base/src/lgapdb04.java" pos="124:3:3" line-data="    public LGAPDB04(Connection connection) {">`LGAPDB04`</SwmToken> is the constructor for the class. It initializes the <SwmToken path="base/src/lgapdb04.java" pos="124:3:3" line-data="    public LGAPDB04(Connection connection) {">`LGAPDB04`</SwmToken> instance with a database connection, which is used for loading base rates and other data during premium calculation.
...
The function <SwmToken path="base/src/lgapdb04.java" pos="131:5:5" line-data="    public void calculatePremium(InputData inputData, CoverageData coverageData,">`calculatePremium`</SwmToken> is the main entry point for premium calculation. It orchestrates the entire calculation process by initializing calculation areas, loading rates, applying modifiers, calculating premiums for each peril, adding catastrophe and expense loads, applying discounts and taxes, and finalizing the premium. This method ties together all the calculation steps and updates the output results structure.
...
The function <SwmToken path="base/src/lgapdb04.java" pos="169:5:5" line-data="    private void p200Init(InputData input, CoverageData coverage, CalculationAreas calc) {">`p200Init`</SwmToken> initializes exposure calculations. It computes the risk adjustment factor based on the input risk score, calculates exposures for building, contents, and business interruption, determines the total insured value, and computes exposure density. It also initializes the base rates array for further calculations.
...
The function <SwmToken path="base/src/lgapdb04.java" pos="202:5:5" line-data="    private void p300LoadRates(InputData input, CalculationAreas calc) throws SQLException {">`p300LoadRates`</SwmToken> loads base rates for each peril from the database using the provided input data. If no rate is found in the database, it uses default rates. The rates are stored in the calculation areas for use in premium calculations.
...
The function <SwmToken path="base/src/lgapdb04.java" pos="245:5:5" line-data="    private void p400ExperienceMod(InputData input, CalculationAreas calc,">`p400ExperienceMod`</SwmToken> calculates the experience modification factor. It adjusts the premium based on the insured's claims history and years in business, applying credibility and capping the modifier within a specified range.
...
The function <SwmToken path="base/src/lgapdb04.java" pos="281:5:5" line-data="    private void p500ScheduleMod(InputData input, CalculationAreas calc,">`p500ScheduleMod`</SwmToken> computes the schedule modification factor. It considers building age, protection class, occupancy hazard, and exposure density to adjust the premium, capping the modifier within defined limits.
...
The function <SwmToken path="base/src/lgapdb04.java" pos="399:5:5" line-data="    private void p700CatLoading(CoverageData coverage, CalculationAreas calc,">`p700CatLoading`</SwmToken> computes catastrophe loading amounts for the premium. It adds loadings for hurricane, earthquake, tornado, and flood perils based on the coverage and calculated premiums.
...
The function <SwmToken path="base/src/lgapdb04.java" pos="453:5:5" line-data="    private void p900Discounts(InputData input, CoverageData coverage,">`p900Discounts`</SwmToken> determines applicable discounts, including multi-peril, claims-free, and deductible credits. It sums and caps the total discount, then calculates the discount amount to be applied to the premium.
...
The function <SwmToken path="base/src/lgapdb04.java" pos="510:5:5" line-data="    private void p950Taxes(OutputResults output) {">`p950Taxes`</SwmToken> calculates the tax amount on the premium by applying the tax rate to the subtotal after discounts.
...
<SwmToken path="base/src/lgapdb04.java" pos="124:3:3" line-data="    public LGAPDB04(Connection connection) {">`LGAPDB04`</SwmToken> is instantiated by passing a database connection object, which it uses to access insurance data for premium calculations. For example, a connection to a DB2 database is established and then passed to <SwmToken path="base/src/lgapdb04.java" pos="124:3:3" line-data="    public LGAPDB04(Connection connection) {">`LGAPDB04`</SwmToken> to create a calculator instance.

After creating an <SwmToken path="base/src/lgapdb04.java" pos="124:3:3" line-data="    public LGAPDB04(Connection connection) {">`LGAPDB04`</SwmToken> instance with the database connection, input data objects are prepared to be used with the calculator for performing advanced actuarial premium computations based on various risk factors and coverage selections.
